              ...
              {categorySpending.length > 0 ? (
                categorySpending.map(item => {
                  ...
                  return (
                    <div 
                      key={item.category} 
                      className="p-3 rounded-xl bg-slate-900/40 border border-white/5 hover:border-white/10 transition-all duration-300 group"
                    >
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2.5">
                          <div className="p-2 rounded-lg bg-slate-800/80 text-cyan-400 group-hover:scale-110 transition-transform">
                        ...
                        <span className={cn("text-[11px] font-bold px-2 py-0.5 rounded-full border font-mono", badgeColor)}>
                          {item.percentage.toFixed(0)}%
                        </span>
                      </div>

                      {/* Custom Progress Bar with smooth fill */}
                ...
                <div className="flex flex-col items-center justify-center h-[200px] text-center border border-dashed border-border/60 rounded-xl p-6">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Click 'Set Budgets' in the navigation header to initialize categories.</p>
                </div>
              )}
            </div>
          </ScrollArea>
